...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (viewBookModal) {
        viewBookModal.addEventListener('show.bs.modal', function(event) {
            ...
            const availableCopiesCount = button.getAttribute('data-book-available-copies');
            ...
            const lostControlNumbers = JSON.parse(button.getAttribute('data-book-lost-control-numbers') || '[]');
            ...
            document.getElementById('modalCopies').textContent = `${availableCopiesCount || '0'} / ${copies || '0'}`;
            ...
            // Filter out lost/damaged copies
            const filterAvailableCopies = controlNumbers.filter(cn => !lostControlNumbers.includes(cn));
            
            if (filterAvailableCopies.length > 0) {
                document.getElementById('copiesCount').textContent = filterAvailableCopies.length;
                filterAvailableCopies.forEach((cn) => {
                    const originalIdx = controlNumbers.indexOf(cn);
                    const status = copyStatus[originalIdx] || 'Available';
                    const badgeClass = status.toLowerCase() === 'available' ? 'bg-success' : 'bg-secondary';
                    const copyIndex = filterAvailableCopies.indexOf(cn);
                    const copyCard = `
